"/home/yasue/Nakano_RNAseq/network_analysis/script/PCCofPCC/ATTEDII_Base/FioreDB/ATTEDII_UMAP_GO&TFtarget&FioreDB.R"
#package----
library(dplyr)
library(stringr)
library(ggplot2)
...
ATTEDII_EnrichemntOfPhynotypeList <- readRDS(file = "/home/yasue/bigdata/yasue/PCCOfPCC/ATTEDII/RDS/Table/EnrichmentAnalysis/ATTEDII_EnrichemntOfPhynotypeList.rds")
ATTEDII_EnrichemntOfPhynotypeList <- ATTEDII_EnrichemntOfPhynotypeList %>% select(-pvalue)
colnames(ATTEDII_EnrichemntOfPhynotypeList) <- c("MCLNum", "Sample", "value")
#processing data----
#Node ≧ 8----
...
T.data <- rbind(allGOTable,
                ...
                ATTEDII_EnrichemntOfPhynotypeList
)
...
T.config$n_epochs <- 1000
T.umap <- umap(T.data, config = T.config)
df.umap <- data.frame(x = T.umap$layout[, 1],
                      y = T.umap$layout[, 2],
                      Sample = rownames(T.data)
)
res <- dbscan(x = df.umap[, 1:2], eps = 0.25, minPts = 3)
...
umap <- umap + geom_point()
umap <- umap + scale_shape_discrete(solid=F)
#umap <- umap + geom_text_repel(size = 1)
#umap <- umap + geom_text(aes(label = rownames(T.data)), size = 2, vjust = 2.5)
ggsave(paste0("Umap_Sample_n_neighbors", T.config$n_neighbors, "dbscan", res$eps, ".png"), umap, width = 8, height = 6)
...
saveRDS(df.umap, "~/UMAPClusterList.rds")