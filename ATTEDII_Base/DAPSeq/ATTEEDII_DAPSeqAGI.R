"/home/yasue/Nakano_RNAseq/network_analysis/script/PCCofPCC/ATTEDII_Base/ATTEEDII_DAPSeqAGI.R"
pwd.family <- list.files("/home/yasue/bigdata/yasue/DAPSeq/dap_data_v4/genes/")
DAPSeq_TF.AGI <- c()
i <- 1
for(i in i:length(pwd.family)){
  pwd.TF <- list.files(paste0("/home/yasue/bigdata/yasue/DAPSeq/dap_data_v4/genes/", pwd.family[i], "/"))
  T.DAPSeq_TF.AGI <- c()
  j <- 1
  for(j in j:length(pwd.TF)){
    pwd <- paste0("/home/yasue/bigdata/yasue/DAPSeq/dap_data_v4/genes/", pwd.family[i], "/", pwd.TF[j], "/chr1-5/chr1-5_GEM_events.nS_targets.txt")
...
saveRDS(DAPSeq_TF.AGI, "/home/yasue/bigdata/yasue/DAPSeq/DAPSeq_TF.AGI.rds")