...
for(g in g:length(unique(DAPSeq.TFName$UMAPCluster))){
  ...
  if(nrow(T.data) > 1){
    ...
    for(h in h:ncol(temp)){
      ...
      title <- paste0("/home/yasue/bigdata/yasue/PCCOfPCC/ATTEDII/RDS/20190809DAPSeq_GenomeWide/UMAPCluster", g, "/DAPSeq_base", str_split(temp[1, h], pattern = "/", simplify = T)[, 2], "&target", str_split(temp[2, h], pattern = "/", simplify = T)[, 2])
      ...
      title <- paste0("/home/yasue/bigdata/yasue/PCCOfPCC/ATTEDII/Image/20190809DAPSeq_GenomeWide/UMAPCluster", g, "/DAPSeq_base", str_split(temp[1, h], pattern = "/", simplify = T)[, 2], "&target", str_split(temp[2, h], pattern = "/", simplify = T)[, 2])
...
#elapsed time----
after <- proc.time()
print(after - before)#68637.774 sec, 1143.963 min, 19.06605 hr, cluster2以外
rm(list = ls())